...
export const UserProfile =({ className ,hiddenName,classNameIcon }) => { 
    return (
           <div className={`user profile  flex items-center  gap-2 ${className} `}>
                ...
                <div className={`info ${hiddenName}  flex-col  hidden md:flex`}>
                <span className={` text-md font-medium hidden md:block `}>John Doe</span>
                <span className={`text-xs text-slate-300 hidden md:block `}>john.doe@example.com</span>
                </div>
            </div>
    )
}